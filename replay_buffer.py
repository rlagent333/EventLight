...
class ReplayMemory(object):
    def __init__(self, max_size, obs_dim, act_dim, num_agents=1):
        ...
        if num_agents == 1:
            ...
            self.obs = np.zeros((max_size, num_agents, obs_dim), dtype='float32')
            if act_dim == 0:  # Discrete control environment
                self.action = np.zeros((max_size, num_agents), dtype='int32')
            else:  # Continuous control environment
                self.action = np.zeros((max_size, num_agents, act_dim), dtype='float32')
            self.reward = np.zeros((max_size, num_agents), dtype='float32')
            self.terminal = np.zeros((max_size, num_agents), dtype='bool')
            self.next_obs = np.zeros((max_size, num_agents, obs_dim), dtype='float32')
    ...
    def load(self, pathname):
        ...
        print("[load rpm]memory loade from {}".format(pathname))
